...
**AI-Assisted Code Modification Engine — written in C17, zero runtime dependencies.**

ArchAgent takes a natural-language change request, generates a validated patch via a local LLM, applies it in an isolated sandbox, builds and tests it, and surfaces a structured JSON result. Everything runs offline.
...
- [Build](#build)
- [Usage](#usage)
  - [AI Patch Workflow](#ai-patch-workflow)
  - [C-to-A64 Playground](#c-to-a64-playground)
  - [Web UI](#web-ui)
...
Run the C-Mini compiler playground:
...
ArchAgent is a systems-level tool written in C17 that translates natural-language change requests into validated, tested patches for C codebases. It orchestrates a 17-step pipeline:
...
```
 ...
 │                    │  from delimited model output
 └─────────┬──────────┘
           │  ParsedResponse
           ▼
 ┌────────────────────┐
 │   diff_parser      │  Unified diff → ParsedDiff
 │                    │  → FilePatch → DiffHunk → DiffLine
 └─────────┬──────────┘
           │  ParsedDiff
           ▼
 ┌────────────────────┐
 ...
 └─────────┬──────────┘
           ...
           ▼
 ┌────────────────────┐
 │     sandbox        │  Creates .archagent/sessions/<id>/context/
 │                    │  Session ID: YYYYMMDD_HHMMSS_<pid>
 └─────────┬──────────┘
           │  Sandbox (paths)
           ▼
 ┌────────────────────┐
 │  patch_applier     │  Fuzzy hunk application (±5 line window,
 │                    │  whitespace-normalised context matching)
 └─────────┬──────────┘
           │
           ▼
 ┌────────────────────┐
...
```
 C-Mini source text
   ...
   AST (AstNode tree)
        ...
        ▼  codegen_generate
        │
   A64 assembly text ──────► session_dir/generated.s
        │
        ▼  execv(assemble)
        │
   Binary ──────────────────► session_dir/program.bin
        │
        ▼  execv(emulate)
        │
   Emulator output ─────────► session_dir/emulator.out
        │
        ▼  parse X00 register
        │
...
| **Sandboxed execution** | All changes happen in an isolated session directory; the original project is never modified unless `--apply` is passed |
| **Patch safety** | Directory-traversal rejection, write-outside-root checks, path canonicalisation |
| **Fuzzy patch application** | ±5-line tolerance window; whitespace-normalised context-line matching |
...
| **C-Mini compiler** | Complete pipeline: lexer → recursive-descent parser → A64 code generator |
| **Flask web UI** | Browser-based interface to the full pipeline and the C-to-A64 playground |
| **14 unit tests** | Covering every module independently, plus one end-to-end integration test |
...
│   ├── test_c2asm_lexer.c
│   ├── test_c2asm_parser.c
│   ├── test_c2asm_codegen.c
│   ├── test_c2asm_pipeline.c
│   ├── test_project_scan.c
│   ├── test_context_pack.c
│   ├── test_diff_parser.c
│   ├── test_patch_applier.c
│   ├── test_patch_validator.c
│   ├── test_response_parser.c
│   ├── test_target_profile.c
│   ├── test_command_runner.c
│   ├── test_integration_mock.c  # End-to-end with mock backend
│   └── test_helpers.h
│
...
│
├── demo_projects/
│   ├── calculator/           # Demo: add exponentiation operator
│   ├── wordcount/            # Demo: fix multi-space/newline word counting
│   └── matrix/               # Demo: optimise traversal for cache locality
│
├── gui/                      # Flask web UI
│   ├── app.py
...
|------------|---------|----------|
| C compiler | `gcc` ≥ 9 or `clang` ≥ 11 (C17) | Yes |
| GNU Make | any modern version | Yes |
| POSIX OS | Linux or macOS | Yes |
| Python 3 + Flask | any recent | Web UI only |
| llama.cpp (`llama-cli`) | any | `--backend llama` only |
| Ollama | any | `--backend ollama` only |

```sh
git clone https://github.com/shiventayal6-bit/archagent.git
cd archagent
make                   # builds bin/archagent
make test              # runs all 14 unit tests
make test-c2asm        # runs C-to-A64 compiler tests only
make clean
```
...
```sh
# No external model: use the built-in mock backend
./bin/archagent \
  --project demo_projects/calculator \
...
# Local quantised model via llama.cpp (fully offline)
./bin/archagent \
  --project my_project/ \
...
Compiles C-Mini to ARMv8 A64 assembly, assembles it, and executes it through the companion emulator.
...
```json
{
  ...
  "stage": "emulate",
  "error": "",
  "session_id": "20260614_153021_98765",
  "session_dir": ".archagent/sessions/20260614_153021_98765/c2asm",
...
```sh
# Compile a .cmini file; print only the generated A64 assembly
...
The GUI exposes the full pipeline and the C-to-A64 playground through a browser interface backed by the same `bin/archagent` binary.

### Demo Targets

```sh
make demo-calculator   # add exponentiation to a calculator (mock backend)
make demo-wordcount    # fix multi-space word counting (mock backend)
make demo-matrix       # optimise matrix traversal + run benchmark (mock backend)
make demo-c2asm        # compile factorial via C-Mini → A64 → emulator
```
...
| Backend | Invocation | Requirements | Privacy |
|---------|-----------|--------------|---------|
| `mock` | `--backend mock` | None | N/A — no model called |
| `llama` | `--backend llama --model /path/to/model.gguf` | [`llama-cli`](https://github.com/ggml-org/llama.cpp) on `$PATH` | Fully offline |
| `ollama` | `--backend ollama --model qwen2.5-coder:7b` | [Ollama](https://ollama.com) on `localhost:11434` | Fully offline |

**Model weight files (`.gguf`) are not included in this repository.** Obtain them separately from a source you are licensed to use.
...
# Download Qwen2.5-Coder 7B Q4 (keep outside the repo)
...
Error handling: if `llama-cli` is not on `$PATH`, or `--model` is absent or points to a nonexistent file, ArchAgent exits cleanly with `"ok": false` and a descriptive message — it never crashes or hangs.

---

## JSON Output Reference

All modes support `--json`. Suitable for piping into `jq` or CI result parsers.
...
```json
{
  ...
  "input_path":    ".archagent/sessions/.../c2asm/input.cmini",
  "assembly_path": ".archagent/sessions/.../c2asm/generated.s",
  "binary_path":   ".archagent/sessions/.../c2asm/program.bin",
  "emulator_output_path": ".archagent/sessions/.../c2asm/emulator.out",
  "result_json_path":     ".archagent/sessions/.../c2asm/result.json",
...
The C-to-A64 Playground compiles a minimal C subset:
...
**Register allocation:** up to 15 user variables, mapped to X1–X15. Temporaries use X16–X25. Return value in X0. Program terminates with `sys_exit` (syscall 93).
...
**Why C17?**
The project targets systems engineers. A C17 implementation demonstrates memory management discipline, explicit ownership, and zero-overhead abstractions. It also produces a single statically-linked binary with no runtime dependency chain.

**Why sandboxing?**
The pipeline applies model-generated patches — code produced by an LLM that may be incorrect. A sandbox means a single bad patch cannot corrupt the original project. All build and test commands run inside the session copy, with an explicit `--apply` flag required to touch the original.

**Why a hand-written parser?**
The C-Mini compiler uses a recursive-descent parser rather than a parser generator (yacc, ANTLR). This keeps the dependency count at zero and makes error messages point to exact source locations without intermediate grammars.

**Why a fuzzy patch applier?**
LLMs sometimes emit diffs with off-by-one line numbers or minor whitespace differences in context lines. A strict applier would fail on every such patch; the fuzzy applier uses a ±5-line search window and whitespace-normalised context matching to absorb these imprecisions.

**Why three backends?**
The mock backend enables fully offline demos and CI with zero setup. The llama backend supports air-gapped operation. The Ollama backend offers a convenient HTTP API without manual llama.cpp compilation. All three implement the same interface — swapping backends is a single flag change.

---

## Module Documentation

Full API reference, data structure descriptions, and data flow diagrams:
...
- **LLVM IR backend for C-Mini** — emit LLVM IR in addition to A64 assembly, enabling portable optimisation
